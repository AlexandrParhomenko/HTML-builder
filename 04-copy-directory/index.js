...
async function copyDir(pathToFilesFolder, pathToCopyFolder) {
  try {
    ...
    for (let file of folderFiles) {
      if (file.isDirectory()) {
        copyDir(path.join(pathToFilesFolder, file.name), path.join(pathToCopyFolder, file.name));
      } else {
...
copyDir(pathToFilesFolder, pathToCopyFolder);